...
@router.get(
    ...
    # Add operation_id to ensure unique route function identification
    operation_id="list_nfts"
)
async def list_nfts(
    ...
    print(f"\n=== list_nfts called with portfolio_id: {portfolio_id} ===")
    if portfolio_id not in nfts_store:
        print(f"Portfolio {portfolio_id} not found in nfts_store")
        return []
    nfts = list(nfts_store[portfolio_id].values())
    print(f"Found {len(nfts)} NFTs for portfolio {portfolio_id}")
    return nfts
...
# Move collections endpoints before the /{nft_id} endpoint to avoid route conflicts
@router.get(
    "/collections",
    response_model=List[NFTCollectionResponse],
    summary="List all NFT collections in a portfolio",
    responses={
        200: {"description": "A list of NFT collections"},
        404: {"description": "Portfolio not found"}
    },
    # Add operation_id to ensure unique route function identification
    operation_id="list_nft_collections"
)
async def list_nft_collections(
    ...
    Args:
        portfolio_id: The ID of the portfolio to list collections for
        current_user: The currently authenticated user
        
    Returns:
        A list of NFT collections in the specified portfolio
        
    Raises:
        HTTPException: If the portfolio is not found
    """
    print(f"\n=== list_nft_collections called with portfolio_id: {portfolio_id} ===")
    print(f"Available portfolio IDs: {list(collections_store.keys())}")
    
    if portfolio_id not in collections_store:
        print(f"Portfolio {portfolio_id} not found in collections_store")
        # Return empty list instead of 404 to match the behavior of list_nfts
        return []
        
    collections = list(collections_store[portfolio_id].values())
    print(f"Found {len(collections)} collections for portfolio {portfolio_id}")
    return collections
